...
class TTS:
    ...
    def generate_audio(self, text, lang='ru'):
        try:
...
async def say_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Enter text. Example: /say Hello world")
        return
        
    text = " ".join(context.args)
    await update.message.reply_text(f"Speaking: {text}")
    
    audio = tts.generate_audio(text, tts.language)
    if audio:
        await update.message.reply_voice(voice=audio, caption=f"Language: {tts.language}")
    else:
...
async def voice_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Voice message received! I can only process text messages.")
